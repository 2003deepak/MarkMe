...
async def get_clerk(department, user_data):
    ...
    cache_key_clerk = f"{user_role}:clerks:{department}"
    ...
    cursor = await clerks_collection.find({"department": department}, exclude_fields)
...
async def get_clerk_by_id(clerk_id, user_data):
    ...
    clerks_collection = get_db().clerks
    clerk = await clerks_collection.find_one({"_id": clerk_id}, exclude_fields)
